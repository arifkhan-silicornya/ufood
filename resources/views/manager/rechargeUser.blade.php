        ...
        <div class="row">
          <div class="col-md-12 pb-5">
            <div class="mx-5 w-75" id="moneyRecharge" style="display: block;">
                <form action="{{ URL('/managerRechargeUser') }}" class="w-100 mx-auto border d-flex flex-row border-info my-5" method="post">
                    @csrf
                <span class="btn btn-info w-25 disabled" disabled> Search User By ID : </span>
...
@isset($user_account)
    ...
    <form method="post" action="{{ url('/managerMoneyRecharge') }}" class="w-75 mx-auto border border-info p-3">
        @csrf
    <div class="form-group border border-info d-none" >
        @php
            $manager = Session::get(md5('manager'));
            $managr = DB::table('manager')->where(['email'=>$manager->email])->first();
        @endphp
        <input value="{{ $managr->email }}" class="form-control " type="email" name="managerEmail" required  readonly  >
    </div>
    ...
    <div class="form-group border border-info" >
        <label class="col-form-label">Current Money :</label>
